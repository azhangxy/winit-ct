...
import sys
sys.path.append(r"D:\causal inference\时序因果推断的可解释性\CausalTransformer")
...
def main(args: DictConfig):
    ...
    # 检查模型参数是否成功移动到指定设备并具有正确的数据类型
    for name, param in multimodel.named_parameters():
        assert param.device == device, f"Model parameter {name} is on device {param.device} instead of {device}"
        assert param.dtype == torch.double, f"Model parameter {name} has incorrect dtype {param.dtype}"
    # 确保所有数据都转换为 torch.Tensor 并移动到同一设备和数据类型
    for dataset in [dataset_collection.train_f, dataset_collection.val_f,dataset_collection.test_cf_one_step, dataset_collection.test_cf_treatment_seq]:
        for key in dataset.data.keys():
            dataset.data[key] = torch.from_numpy(dataset.data[key]).to(device).to(torch.double)
    '''
    if args.model.multi.tune_hparams:
        multimodel.finetune(resources_per_trial=args.model.multi.resources_per_trial)
    # 在模型初始化后检查模型参数类型
    for param in multimodel.parameters():
        assert param.dtype == torch.double, f"Model parameter has incorrect dtype {param.dtype}"
